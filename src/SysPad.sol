...
contract SysPad is TierSystem, Pausable {
    ...
    // Mapping from token to array of campaign address.
    mapping(address => address[]) public getCampaignsByToken;
     ...
     * @return length Return number of created campaigns
     */
    function allCampaignsLength() external view returns (uint256 length) {
        length = allCampaigns.length;
    }

    /**
     * @notice Retrieve number of campaigns created for specific token
     * @param _token Address of token want to query
     * @return length Return number of created campaign
     */
    function getCampaignsLengthByToken(
        address _token
    ) public view returns (uint256 length) {
        length = getCampaignsByToken[_token].length;
    }
    ...
    function registerSale(
        ...
        require(_usdConversionRate > 1e6, "SysPad::INVALID_USD_RATE");
        require(
        ...
        getCampaignsByToken[_token].push(address(sale));
        allCampaigns.push(address(sale));
